...
class PublishGradlePluginTests {
	...
	void runWhenReleaseTypeMilestoneShouldDoNothing() throws Exception {
		this.command.run(new DefaultApplicationArguments("publishGradlePlugin", "M", getBuildInfo()));
		verifyNoInteractions(this.service);
	}

	@Test
	void runWhenReleaseTypeRCShouldDoNothing() throws Exception {
		this.command.run(new DefaultApplicationArguments("publishGradlePlugin", "RC", getBuildInfo()));
		verifyNoInteractions(this.service);
	}
	...
	void runWhenReleaseTypeReleaseShouldCallService() throws Exception {
		...
		this.command.run(new DefaultApplicationArguments("promote", "RELEASE", getBuildInfo()));
