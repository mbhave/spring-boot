...
public class ReactiveCloudFoundryActuatorAutoConfiguration {
	...
	public CloudFoundryWebFluxEndpointHandlerMapping cloudFoundryWebFluxEndpointHandlerMapping(
			OperationParameterMapper parameterMapper, EndpointMediaTypes endpointMediaTypes,
	...
	static class SecurityConfiguration {
		...
		WebFilterChainProxy postProcess(WebFilterChainProxy original) {
			ServerWebExchangeMatcher matcher = ServerWebExchangeMatchers.pathMatchers("/cloudfoundryapplication/**");
			WebFilter filter = (exchange,chain) -> chain.filter(exchange);
			MatcherSecurityWebFilterChain ignoredRequests = new MatcherSecurityWebFilterChain(matcher, Collections.singletonList(filter));
			MatcherSecurityWebFilterChain chainWithOriginal = new MatcherSecurityWebFilterChain(
					ServerWebExchangeMatchers.anyExchange(), Collections.singletonList(original));
			return new WebFilterChainProxy(ignoredRequests, chainWithOriginal);
		}
	}
