...
public class SpringBootTestRandomPortEnvironmentPostProcessor
	...
	public void postProcessEnvironment(ConfigurableEnvironment environment,
		...
		MapPropertySource testPropertySource = (MapPropertySource) environment
				.getPropertySources()
				.get(TestPropertySourceUtils.INLINED_PROPERTIES_PROPERTY_SOURCE_NAME);
		if ("0".equals(testPropertySource.getProperty(SERVER_PORT_PROPERTY))) {
			if (testPropertySource.getProperty(MANAGEMENT_PORT_PROPERTY) == null) {
				...
				if (managementPort != null && !"-1".equals(managementPort)) {
					if (!managementPort.equals(serverPort)) {
						testPropertySource.getSource().put(MANAGEMENT_PORT_PROPERTY, "0");
					}
					else {
						// TODO do something to the management port so that there is no
						// child context
					}
				}
			}
	...
	private String getPort(ConfigurableEnvironment environment, String property,
		...
		return environment.getPropertySources().stream()
				.filter(propertySource -> !propertySource.getName().equals(
						TestPropertySourceUtils.INLINED_PROPERTIES_PROPERTY_SOURCE_NAME))
				.map(ps -> (String) ps.getProperty(property)).filter(Objects::nonNull)
