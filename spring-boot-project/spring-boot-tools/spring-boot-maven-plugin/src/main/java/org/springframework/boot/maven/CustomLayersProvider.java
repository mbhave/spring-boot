...
import org.springframework.boot.loader.tools.layer.classes.CustomResourceStrategy;
...
import org.springframework.boot.loader.tools.layer.classes.ResourceStrategies;
...
import org.springframework.boot.loader.tools.layer.library.CustomLibraryStrategy;
import org.springframework.boot.loader.tools.layer.library.LibraryFilter;
import org.springframework.boot.loader.tools.layer.library.LibraryStrategies;
...
public class CustomLayersProvider {
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			if (item instanceof Element) {
				...
				if ("strategy".equals(element.getTagName())) {
					...
					strategy.add(new CustomLibraryStrategy(layer, filters));
				}
			}
		}
	...
	private ResourceStrategies getResourceStrategies(NodeList strategies) {
		...
		for (int i = 0; i < strategies.getLength(); i++) {
			...
			if (item instanceof Element) {
				...
				if ("default-strategy".equals(element.getTagName())) {
					try {
						ResourceStrategy resourceStrategy = ResourceStrategies.defaultStrategy(id, layer);
						strategy.add(resourceStrategy);
					}
					catch (Exception ex) {
						throw new IllegalArgumentException("No default strategy found for id '" + id
								+ "' and custom strategy definition must not be empty.");
				...
				if ("strategy".equals(element.getTagName())) {
					...
					strategy.add(new CustomResourceStrategy(layer, filters));
				}
			}
		}
	...
	private void addFilter(List<ResourceFilter> filters, Node filter, List<String> includeList,
		...
		if ("locations".equals(filter.getNodeName())) {
			filters.add(new org.springframework.boot.loader.tools.layer.classes.LocationFilter(includeList, excludeList));
		}
	}
